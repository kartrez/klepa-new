...
export const layer: Layer.Layer<Service, never, Core.Service | Config.Service | ModelCache.Service> = Layer.effect(
  Service,
  Effect.gen(function* () {
    ...
    const get = Effect.fn("ModelsDev.get")(function* () {
      ...
      const models = yield* cache.refresh(GPT_CHAT_BY_PROVIDER_ID, fetch).pipe(Effect.catch(() => Effect.succeed({})))
...
export const node = LayerNode.suspend(() => LayerNode.make(layer, [Core.node, Config.node, ModelCache.node]))
